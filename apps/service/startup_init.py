...
def startup_push_recent_trades():
    ...
    try:
        ...
        # 获取交易数据
        result = monitor_service.fetch_trades()

        if not result["success"]:
            logger.error(f"获取交易数据失败: {result.get('error')}")
            return
        ...
        logger.info(f"获取到 {len(trades)} 条交易，准备推送最近 {actual_count} 条")
        ...
        logger.info(f"启动推送完成，已推送 {actual_count} 条交易")
        logger.info("=" * 50)
